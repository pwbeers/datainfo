...
public abstract class CatalogPage extends HttpServlet {
	 ...
	 * contains a short description, a long description, and a price, using the
	 * item ID as the unique key.
	 * <P>
	 * Servlets that extend CatalogPage <B>must</B> call this method (usually
	 * from init) before the servlet is accessed.
	 */
	...
	/**
	 * Sets the page title, which is displayed in an H1 heading in resultant
	 * page.
	 * <P>
	 * Servlets that extend CatalogPage <B>must</B> call this method (usually
	 * from init) before the servlet is accessed.
	...
	/**
	 * First display title, then, for each catalog item, put its short
	 * description in a level-two (H2) heading with the price in parentheses and
	 * long description below. Below each entry, put an order button that
	 * submits info to the OrderPage servlet for the associated catalog entry.
	 * <P>
